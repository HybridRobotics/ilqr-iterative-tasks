...
def kinetic_bicycle(x, u, dt):
    ...
    x_next = np.array(
        [
            x[X_ID["x"]]
            + np.cos(x[X_ID["theta"]]) * (x[X_ID["v"]] * dt + (u[U_ID["accel"]] * dt ** 2) / 2),
            x[X_ID["y"]]
            + np.sin(x[X_ID["theta"]]) * (x[X_ID["v"]] * dt + (u[U_ID["accel"]] * dt ** 2) / 2),
...
def get_A_matrix(velocity_vals, theta, acceleration_vals, num_horizon, dt):
    ...
    A = np.array(
        [
            [o, z, cos(theta) * dt, -(v * dt + (v_dot * dt ** 2) / 2) * sin(theta)],
...
def get_B_matrix(theta, num_horizon, dt):
    ...
    o = np.ones((num_horizon))
    B = np.array(
        [[dt ** 2 * cos(theta) / 2, z], [dt ** 2 * sin(theta) / 2, z], [dt * o, z], [z, dt * o]]
    )
    return B
